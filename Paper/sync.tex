...
\begin{abstract}
 ...
 Tidal torques dominate the angular momentum evolution of short-period, low-mass stellar binaries, but few previous studies have examined tides in binaries out to intermediate orbital periods (P$_{orb}$). Here, we examine how tidal torques, stellar evolution, and magnetic braking shape the rotation period (P$_{rot}$) evolution of low-mass stellar binaries with P$_{orb} \leq 100$ d by performing Monte Carlo simulations of coupled stellar-tidal evolution. We find that tides can modify stellar rotations in binaries with orbital periods (P$_{orb}$) up to 100 days.  At short P$_{orb}$, many stellar binaries tidally-lock into synchronous rotation, naturally producing a population of fast rotators (P$_{rot} < 10$ d) as is observed in the \textit{Kepler} field, a feature that cannot be reproduced by single-star-only models of magnetic braking.  Many binaries with longer P$_{orb}$ can tidally-lock, fixing the stellar P$_{rot}$ to the binary P$_{orb}$, causing P$_{rot}$ to not be a valid proxy for age in all cases, i.e. gyrochronology methods must be applied carefully. We compare our model predictions with \kepler eclipsing binary (EB) rotation periods and find that our predictions are in excellent agreement with observations, with the ``Constant Time Lag" equilibrium tidal model best matching observations. We recommend observational tests to discriminate between equilibrium tidal models for stars.  We show that when tidal torques are weak, even in short P$_{orb}$ binaries, tidal torques can overpower magnetic braking, producing a long-lasting population of subsynchronous rotators similar what \citet{Lurie2017} observed in their sample of \kepler EBs. By comparing observations of \kepler EBs with our simulations, we loosely constrain low-mass stellar tidal time lags to $\tau = 0.19^{+0.43}_{-0.16}$ s.
...
In Fig.~\ref{fig:lurie7}, we display P$_{orb}/$P$_{rot}$ as a function of P$_{orb}$ for both the CPL and CTL models where each simulation was integrated to an age uniformly sampled over $1-7$ Gyr, consistent with ages of \kepler field stars \citep{Chaplin2014}.  Both models reproduce most trends in the \citet{Lurie2017} data: the large cluster of synchronized rotators with P$_{orb} \lsim 20$ d, a population of subsynchronous rotators with P$_{orb} \lsim 20$ d, and the sparse cluster of supersynchronous rotators at all P$_{orb}$ that correspond to young stars that have yet to significantly spin down due to magnetic braking or tides.  The lower limit seen in both panels is a line of nearly constant P$_{rot} \approx 60$ d set by how much a star can spin down via magnetic braking in the weak tides regime over 7 Gyr, the longest age considered in our simulations. 
...
Although \citet{Lurie2017} argues that differential rotation creates the subsynchronous population, we find that the competition between weak tidal torques and magnetic braking naturally
produces the population.  Our mechanism for producing long-lasting subsynchronous rotation described in $\S$~(XXX sec ref) does produce the population seen in Fig.~\ref{fig:subsync} as the sunsynchronous rotators systematically have weaker tidal torques, with a median $\tau = 0.13$ s, compared to the median $\tau = 0.08$ s for the supersynchronous population in which strong tides tidally-lock the binaries.  

Our CTL model successfully reproduces the \citet{Lurie2017} data not just by qualitatively matching the spin-orbital populations observed in \kepler, but also in specific populations. \citet{Lurie2017} find that $15\%$ of their subsynchronous population with 2 $<$ Porb $<$ 10 days has P$_{orb}/$P$_{rot} \in [0.84, 0.92]$, compared with $11\%$ of our CTL population, suggesting that subsynchronous rotation is a common outcome of stellar-tidal evolution in low-mass binaries. Furthermore in the same P$_{orb}$ regime, \citet{Lurie2017} find that $72\%$ of their population has P$_{orb}/$P$_{rot} \in [0.84, 0.92]$, compared with $61\%$ of our CTL population.   Both are close matches, especially given that we did not hand-tune our simulation initial conditions, suggesting that our coupled stellar-tidal model using the CTL formalism is an accurate description of the physics of tidally-interacting binaries. These findings suggest that the CTL model describes tidal interactions in low-mass binaries better than the CPL model, but in the absence of reliable uncertanties on the \citet{Lurie2017} data, we cannot definitively assert that in general, the CTL model is superior to the CPL model for tidal torques in low-mass binaries.  We discuss means to discriminate between the two models in $\S$~\ref{sec:whichModel}.  

\subsubsection{$Q$, $\tau$ Constraints from P$_{orb}$ and P$_{rot}$} \label{sec:qTau}

XXX: say how our quoted $Q$ and $\tau$ ranges produced binaries consistent with what \kepler saw, so not only is our model a reasonable choice to model stellar-tidal evolution, but also our quoted tidal parameter ranges seem to bracket what stars actually have.  We refine it in the below analysis
...
-hammer implications for gyrochronology, fast rotators in kepler field, compare to \citet{Simonian2018}

- \citet{Abt2004} find evidence for tides modifying P$_{rot}$ in binaries with P$_{orb}$ up to 500 d.
...
Here we offer observational tests to discriminate between the two models in short P$_{orb}$ binaries where tidal effects are more likely to be significant.  First, one could obtain a series of time-resolved spectra of the short P$_{orb}$ supersynchronous binaries studied by \citet{Lurie2017} to derive more robust $e$ constraints than derived using the approximations in \citet{Lurie2017}.  For binaries with P$_{orb} < 10$ and $e < \sqrt{1/19}$, the CPL model does not permit supersynchronous rotation except in extremely young systems that are still contracting along the pre-main sequence, or that have just reached the main sequence, while the CTL model does allow for it.  If supersynchronous rotation is observed in such systems, it is evidence in favor of the CTL model.  

Second, for tidally-locked binaries with $e > \sqrt{1/19}$ for short P$_{orb}$, the CPL model predicts supersynchronous rotation in the form of a 3:2 spin-orbit comensurability, e.g. the line at P$_{orb}/$P$_{rot} = 1.5$ seen in the left panel of Fig.~\ref{fig:lurie7}, and no other spin state is permited.  The CTL model, however, permits a continuum of supersynchronous states in eccentric tidally-locked rotators, e.g. right panel of Fig.~\ref{fig:lurie7}, that seems to provide a much better match to the \citet{Lurie2017} data, providing strong evidence in favor of the CLT model. The lack of uncertainties on the \citet{Lurie2017} P$_{rot}$ measurements and their poor eccentricity constrains, however, prohibit us from making this distinction conclusive in this work.  Again, more robust $e$ constrains via time-dependent spectra would aid in this discriminaton between models, as would determinations of P$_{rot}$ with uncertainties, perhaps using a gaussian process model as in (XXX cite GP rasumussen, and correct ruth angus paper).  Accurate determinations of the spin-orbital state of short P$_{orb}$ binaries are critical to determine which tidal model best describes the evolution of low-mass stellar binaries.
...
presence of 3:2 cluster would be strong evidence in favor of CPL model.  Can use presence/absence of this feature as a strong discriminators between tidal models.  additionally, CTL predicts more subsync clusters that could be used for model descriptions. below, say how CPL model has locked binaries at long Porb, ctl doesn't above 20 d so that's another model descriimination.  say each time that CTL passes, CPL doesn't according to lurie data.  In general, talk about implications and use flowery language

- talk about how we need more binary observations with Prot, Porb, and ecc, especially at longer orbital periods, to more reliably discriminate between tidal models.  At low eccentricity, it does matter (sync vs supersync in \kepler) as opposed to what \citet{Leconte2010} recommends
