...
\subsection{The Rotational Equilibrium State} \label{sec:eq}

we actually see subsync rotation in Kepler-47, RUP whatever it's number (short period binaries!), and in the Kepler field!

binaries WILL tidally-lock in the long term as P$_{eq}$ will decrease because P$_{orb}$ decreases in the long-run due to tidal energy dissipation (e.q. Eqn.~(\ref{eqn:tidal_locked_two}), adot from Fleming+2018 coupled stellar-tidal evolution, reference example tidal example above, Fleming+2018 paper).  As P$_{orb}$ decreases, closer to P$_{rot}$ so easier locking, AND tidal torques become strong as distance decreases, allowing them to overpower magnetic braking.  Can see this is slope of P$_{eq}/$P$_{rot}$ curve steepening.  Dissipation occurs because weak tidal torques want to balance out magnetic braking, keeping Prot not equal to Peq, producing tidal dissipation. Tides plus magnetic braking still constantly spinning star down. Systems WILL inevitably tidally-lock, but at MUCH longer timescales as most of our Peq simulations do not lock by 7 Gyr even though Prot=Peq passings occured, but not locking because magnetic braking overpowered tidal torques at that time.
...
\begin{figure*}[t]
   ...
   \caption{P$_{orb}/$P$_{rot}$ as a function for P$_{orb}$ simulated according to the CPL model (left) and the CTL model (right) colored by $e$, compared with \kepler EBs from \citet{Lurie2017} (red plus signs).  Each simulation is integrated to system ages uniformly sampled over $1-7$ Gyr, consistent with ages of \kepler field stars \citep{Chaplin2014}. The \kepler EBs at low P$_{orb}$ and low P$_{orb}/$P$_{rot}$ are likely either brown dwarfs or exoplanets \citep{Lurie2017}, and hence not modeled by our simulations, so we do not consider them. TODO.}%
    \label{fig:lurie7}%
\end{figure*}
...
\begin{figure}
   ...
   \caption{Same format as Fig.~\ref{fig:lurie7}, with \citet{Lurie2017} data plotted as diamonds.  All points are colored according to the binary orbital $e$. Our simulations are in excellent agreement with the observed orbital and rotational states of \kepler EBs. Our model predicts that nearly all EBs with P$_{orb} < 3$ d are synchronized as is observed by \citet{Lurie2017}. We find that our model predicts a population of supersyncronous rotators on eccentric orbits and a population of subsyncronous rotators with P$_{orb}/$P$_{rot} \approx 0.9$ on nearly-circular orbits, both of which were observed by \citet{Lurie2017}. This excellent agreement between theory and observations validates our model and provides strong evidence that the CTL formalism can accurately model the tidal evolution of low-mass stellar binaries.}%
    \label{fig:subsync}%
\end{figure}
